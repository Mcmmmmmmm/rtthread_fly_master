...
void led_on(void)
{
    rt_pin_mode(112,PIN_MODE_OUTPUT);
    rt_pin_write(112,PIN_LOW);
}
...
//    rt_timer_control(timer_test, RT_TIMER_CTRL_SET_TIME, (void *)&timeout_value);
    ...
    if(timeout_value == 10)
    {
        rt_pin_mode (134,PIN_MODE_OUTPUT);
        rt_pin_write(134,PIN_LOW);
        led_state = 1;
...
int rt_led_init(void)
{
    ...
    thread = rt_thread_create("led", led_run, RT_NULL, 2048, 2, 50);
